...
public:
    ...
    EdgeSetGraph(size_t verts, size_t edges, std::vector<std::set<size_t>> neighbours) : verts_(verts), edges_(edges), neighbours_(neighbours) {};
    size_t vertCount();
    size_t edgeCount();
    ...
    std::vector<std::set<size_t>> getNeighbourList();
    
private:
    size_t verts_;
    size_t edges_;
    std::vector<std::set<size_t>> neighbours_;
};
